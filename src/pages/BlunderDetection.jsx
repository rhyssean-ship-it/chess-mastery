...
import blunderDetection from '../data/blunderDetection';
...
export default function BlunderDetection() {
  ...
  const filtered = useMemo(() => blunderDetection.filter(b => b.difficulty === difficulty), [difficulty]);
  ...
  return (
    <div className="page-enter max-w-5xl mx-auto px-4 sm:px-6 py-10">
      <h1 className="text-3xl font-display text-gold mb-1">Blunder Detection</h1>
      <p className="text-text-dim text-base mb-6">A move was just played. What did they blunder?</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_300px] lg:grid-cols-[1fr_380px] gap-6 sm:gap-8">
        <div className="w-full max-w-[480px]">
          <ChessBoard fen={puzzle.fen} movable={false} />
        </div>
        <div className="space-y-4">
          <div className="card-base p-5">
            <p className="text-sm text-text-dim mb-1">Last move played: <span className="text-incorrect font-mono">{puzzle.lastMove}</span></p>
            <p className="font-semibold">{puzzle.question}</p>
          </div>

          <div className="card-base p-4">
            <span className={`text-xs px-2 py-0.5 rounded-full ${diffColors[puzzle.difficulty]}`}>{puzzle.blunderType}</span>
          </div>
          ...
          {!revealed ? (
            ...
            <>
              <div className="card-base p-5 border-gold/20">
                ...
                <p className="text-sm text-text-dim">Punishment: <span className="text-gold font-mono">{puzzle.punishment}</span></p>
              </div>
              <p className="text-sm text-text-dim text-center">Did you find the blunder?</p>
